...
(function(document, Granite, $, undefined) {
    ...
    function errorUpdatingUser(data) {
        ...
        if (data) {
            ...
            if (parsedResponse && parsedResponse.headers.Message) {
			    if(parsedResponse.headers.Message.indexOf("Password violates password constraint") < 0){
					response = parsedResponse.headers.Message;
				} else {
					response =  Granite.I18n.get("BNP Paribas Password Constrain Message")
				}
            }
        }
        currentState = "init";
    ...
    */
    ...
    function createPropertiesFormData(form) {
        ...
        var formData = form.find("input[name!='" + disableAccountFormFieldName + "'][name!='rep:re-password'],textarea,select[name!='" + disableAccountFormFieldName + "']").serialize() + enableDisableUser + "&" + CUI.AuthorizableUtils.CHARSET_ENCODING_XHR_PARAM;
    ...
    */
    ...
    /*
     * Register the click event handler for the Close button on the user properties form
     */
