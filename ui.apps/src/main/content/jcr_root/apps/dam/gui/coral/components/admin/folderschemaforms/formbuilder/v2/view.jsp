...
%><cq:defineObjects /><%
    ...
    String formPathOOTB =  DamConfigurationConstants.OOTB_FOLDER_METADATA_SCHEMA_FORM_HOME;
    if(formPathRes != null){
        Conf conf = formPathRes.adaptTo(Conf.class);
        formPathOOTB = conf.getItem(DamConfigurationConstants.ADMIN_UI_OOTB_CONF_RELPATH).get(DamConfigurationConstants.FOLDER_METADATA_SCHEMA_HOME, DamConfigurationConstants.OOTB_FOLDER_METADATA_SCHEMA_FORM_HOME);

    }
    String formFieldsPath = cfg.get("formfields", "/libs/dam/gui/coral/components/admin/folderschemaforms/formbuilder/formfields");

    // v2 is meant for only foldermetadataschemaeditor
...
%><section class="formbuilder-tab-section coral-TabPanel-pane" role="tabpanel">
    ...
    %><fieldset>
        ...
        %><li data-id="<%= xssAPI.encodeForHTMLAttr(col1Item.getName()) %>" class="field" data-fieldtype="<%= xssAPI.encodeForHTMLAttr(type) %>">
