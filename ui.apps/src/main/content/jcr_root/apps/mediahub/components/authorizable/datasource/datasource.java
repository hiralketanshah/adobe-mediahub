package apps.mediahub.components.authorizable.datasource;
...
public class datasource extends SlingSafeMethodsServlet {
    ...
    private static <T> T createTermCondition(String query, QueryBuilder<T> builder) {
        ...
        if (StringUtils.isNotBlank(query)) {
            ...
            T termCondition = builder.or(builder.like("fn:lower-case(@rep:principalName)", pattern),
                builder.or(builder.like("fn:lower-case(" + PATH_GIVEN_NAME + ")", pattern),
                    builder.or(builder.like("fn:lower-case(" + PATH_FAMILY_NAME + ")", pattern),
                        builder.or(
                            builder.like("fn:lower-case(" + PATH_DISPLAY_NAME + ")", pattern),
                            builder.like("fn:lower-case(" + PATH_EMAIL + ")", pattern)
                        )
                    )
                )
            );
