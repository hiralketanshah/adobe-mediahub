...
public class UpdateInternalUsersScheduler implements Runnable {
    ...
    public void run() {
        ...
        if (isEnabled && this.slingSettingsService.getRunModes().contains("author")) {
        	updateInternalUsersService.createAndUpdateUsers(csvUserInfo,csvAdditionalInfo);
        	LOGGER.debug("Internal Users are successfully created/updated or deleted as per the records present in the latest CSV file");
